...
exports.parseList = function(url, config, callback) {
    request({ 'url' : url, 'encoding' : 'binary', 'timeout' : 50000 }, function(error, response, body) {
        if (error) {
            console.log(error);
            return;
        ...
        //console.log(body);
        var parseList = require("./parser").parseList;
        parseList(body, {config:config}, function(error, match){
            if (error) {
                console.log(error);
                return;
            }
...
exports.parseBaiduList = function(url, callback) {
    request({ 'url' : url, 'encoding' : 'binary', 'timeout' : 50000 }, function(error, response, body) {
        if (error) {
            console.log(error);
        }
