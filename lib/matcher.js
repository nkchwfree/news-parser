...
function getTitle($) {
    ...
    var list = $('h1,h2,h3,h4,h5,div,span,p').filter(function(index){
        var self = $(this);
        var id = self.attr('id') || "";
        var class_name = self.attr('class') || "";
        var pattern = /(title|headline)/i;
        //console.log(self[0].tagName);
        return self[0].tagName.match(/^h\d/i) || id.match(pattern) || class_name.match(pattern);
    })
    var dom;
    ...
    for(var i=0; i< list.length; i++) {
        ...
        var title = dom.text().trim().replace(/(\(\d+\))$/,"");
