...
function getTitle($) {
    ...
    function _filter_dom(dom) {
        ...
        var pattern = /(title|main|headline|tit|text|tt|bt|ReportIDname)/i;
        //console.log(self.attr('style'));
        //return self[0].tagName.match(/^h\d/i) || id!="" || class_name !="" || self.css('font-family')=='黑体';
    ...
    //var title = "";
    /*var list = $('h1,h2,h3,h4,h5,div#headline,div[class*="title"],div[class*="Title"],div[id*="Title"],div[id*="title"],span.title,span#title,').filter(function(index){
         return $('span,div,table', this).length == 0;
    })*/
    var list = $('h1,h2,h3,h4,h5,div,span,p,td').filter(function(index){
        //console.log($(this).find('font:first').html());
    ...
    for(var i=0; i< list.length; i++) {
        ...
        //判断是否是标题
        if(title.length>0 && page_title.indexOf(title)>=0) {
...
function getContentByList($) {
    ...
    $("div,span").filter(function(index){
        ...
        return $('span,div,table', this).text().trim().length == 0 && !id.match(pattern) && !class_name.match(pattern);
    }).each(function(){
        var dom = $(this);
        console.log('----------------------------------------');
        console.log(dom.html());
